...
**Signature carousel eligibility ≠ attendance list.** The "Collect Signatures" carousel must build its attendee list from the **full static roster**, excluding only people whose signature status is explicitly `absent`. Do NOT filter it by `isAttending`/the attendance union: Teams self-signs write a *partial* attendance set (signers only), so before anyone opens "Manage Attendance" the union exists but is incomplete, and `isAttending`'s "not in list ⇒ absent" branch wrongly hid every unsigned person — they only reappeared after visiting Manage Attendance (which locally `initializeAttendance`s everyone, lost on refresh). `isAttending` is for the Manage-Attendance checkboxes only.
...
**Attendee NAME divergence is a second silent-absent trap (separate from ISO keys).** Signatures/attendance are also keyed by the person's exact roster name. The Teams signer is resolved to the canonical roster (`shared/meetingRoster.ts`, e.g. "Daniel Conlan"), so signatures are stored under that name. Any render/export path that keeps its OWN hardcoded attendee list (e.g. "Dan Conlan") will fail the name lookup and show a signed person as absent. **Rule:** every attendee-rendering path must source names from `allRosterMembers`, never a local copy. The HTML export's `generateAttendanceSection` had this bug; Word (`server/template-engine.ts`) and Markdown (`server/markdown-generator.ts`) generators STILL carry stale independent lists (with "Isaac Ensor", no "Daniel Conlan") — not wired to the meeting-history UI today, but fix them to the shared roster if ever re-enabled.
