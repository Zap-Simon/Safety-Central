...
import { Leaf, Target, type LucideIcon } from "lucide-react";
...
interface EnvironmentQualityCard {
  id: string;
  title: string;
  description: string;
  icon: LucideIcon;
...
const defaultEnvironmentQualityCards: EnvironmentQualityCard[] = [
  {
    id: "environment-policy",
    title: "Environment Policy",
    description: "Environmental policies & procedures",
    icon: Leaf,
    iconColor: "text-green-600",
    backgroundColor: "bg-green-100",
    href: "/environment-policy"
  },
  {
    id: "quality-policy", 
    title: "Quality Policy",
    description: "Quality standards & management",
    icon: Target,
...
export default function EnvironmentQualityTab() {
  const [cards, setCards] = useState<EnvironmentQualityCard[]>(defaultEnvironmentQualityCards);
  ...
  // Load card ordering from the database for environment-quality cards
  const { data: cardOrdering } = useQuery({
    queryKey: ['/api/environment-quality-card-ordering']
  });

  // Save card ordering to the database for environment-quality cards
  const saveCardOrderingMutation = useMutation({
    mutationFn: async (cardOrders: { cardId: string; position: number }[]) => {
      const response = await apiRequest('POST', '/api/environment-quality-card-ordering', { cardOrders });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/environment-quality-card-ordering'] });
    }
  });
  ...
  useEffect(() => {
    if (cardOrdering && (cardOrdering as any)?.success && (cardOrdering as any).cardOrdering && !hasLoadedFromDB) {
      ...
      if (savedOrder.length > 0) {
        ...
        const orderedCards = [...defaultEnvironmentQualityCards].sort((a, b) => {
        ...
        setCards([...defaultEnvironmentQualityCards]);
        setHasLoadedFromDB(true);
      }
    }
  ...
  const renderCard = (card: EnvironmentQualityCard, index: number) => {
    ...
    if (card.requiresAuth && card.protectedPath) {
      const handleProtectedClick = async () => {
        ...
        try {
          if (card.id === 'environment-policy') {
            await authService.openHealthSafetyPolicy();
          } else if (card.id === 'quality-policy') {
            await authService.openHealthSafetyPolicy();
          }
        } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Environment & Quality Navigation */}
