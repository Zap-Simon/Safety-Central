...
import { Target, Download, FileText, File, ExternalLink } from "lucide-react";
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-gray-600 text-xs sm:text-sm truncate">Quality Policy</p>
            </div>
          </div>
...
export default function QualityPolicy() {
  ...
  // Fetch quality policy documents from the folder with authentication
  const { data: policyDocuments, isLoading, error } = useQuery({
    queryKey: ['/api/quality-policy-documents'],
    queryFn: async () => {
      try {
        const token = await authService.getAccessToken();
        const response = await fetch('/api/quality-policy-documents', {
          headers: {
        ...
        if (!response.ok) {
          ...
          console.error('Quality policy documents API error:', response.status, errorText);
          throw new Error(`Failed to fetch quality policy documents: ${response.status} - ${errorText}`);
        }
        ...
        <div className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 mb-6 border border-gray-200">
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-lg sm:text-2xl font-bold text-gray-900 leading-tight">Quality Policy</h1>
                <p className="text-xs text-gray-600 hidden sm:block">Quality management systems and standards</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  <h2 className="text-lg font-semibold text-gray-900">Current Quality Policy Documents</h2>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="text-gray-500">Loading quality policy documents...</div>
                  </div>
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center space-x-3 flex-1 min-w-0">
                            {document.type === 'word' ? (
                              ...
                              <File className="text-blue-600 flex-shrink-0" size={20} />
                            )}
                            <div className="min-w-0 flex-1">
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 mb-4">
                      Quality policy documents will appear here when available.
                    </p>
                    <p className="text-sm text-gray-400">
                      Check back later for updated quality policy documents.
                    </p>
                  </div>
                )}
              </CardContent>
