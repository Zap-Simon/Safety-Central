...
import { Leaf, Download, FileText, File, ExternalLink } from "lucide-react";
...
function EnvironmentPolicyHeader() {
  return (
...
export default function EnvironmentPolicy() {
  ...
  // Fetch environment policy documents from the folder with authentication
  const { data: policyDocuments, isLoading, error } = useQuery({
    queryKey: ['/api/environment-policy-documents'],
    queryFn: async () => {
      try {
        const token = await authService.getAccessToken();
        const response = await fetch('/api/environment-policy-documents', {
          headers: {
        ...
        if (!response.ok) {
          ...
          console.error('Environment policy documents API error:', response.status, errorText);
          throw new Error(`Failed to fetch environment policy documents: ${response.status} - ${errorText}`);
        }
        ...
        <div className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 mb-6 border border-gray-200">
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-lg sm:text-2xl font-bold text-gray-900 leading-tight">Environment Policy</h1>
                <p className="text-xs text-gray-600 hidden sm:block">Environmental management policies and procedures</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>
                  <h2 className="text-lg font-semibold text-gray-900">Current Environment Policy Documents</h2>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="text-gray-500">Loading environment policy documents...</div>
                  </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 mb-4">
                      Environment policy documents will appear here when available.
                    </p>
                    <p className="text-sm text-gray-400">
                      Check back later for updated environment policy documents.
                    </p>
                  </div>
                )}
              </CardContent>
