...
export class AuthService {
  ...
  /**
   * Navigate to the authenticated Health & Safety policy document
   */
  async openHealthSafetyPolicy(): Promise<void> {
    try {
      ...
      form.method = 'GET';
      form.action = '/policy/health-safety';
      form.target = '_blank';
      
      // Add authorization header via hidden input (we'll handle this server-side)
      ...
      console.error('Failed to open Health & Safety policy:', error);
      throw error;
    }
  }
}
